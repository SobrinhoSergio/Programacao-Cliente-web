    ...
    adicionar.addEventListener('click', function() {
        try {
            ...
            if (codigo.value.substring(0, 1) == 'P' && item_venda.getComercializavel().getEstoque() < quantidade.value) {
                throw new DominioException('Quantidade indisponivel de estoque. Disponível:' + item_venda.getComercializavel().getEstoque());
            } else {
...
concluir.addEventListener('click', function() {
    try {
        if (array_item.length == 0 || array_item.length == []) {
            throw new DominioException('Não pode concluir uma venda sem nenhum item selecionado.');
        }
